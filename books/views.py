...
def search_books(request):
    ...
    # Fetch data if there's a search query or if we want to show all results
    if query:
        graphql_query = {
            "query": """
                query ($search: String) {
        ...
        # Fetch default anime list if no query
        graphql_query = {
            "query": """
                query {
                    Page {
    ...
    try:
        response = requests.post(ANILIST_GRAPHQL_URL, json=graphql_query)
        ...
        anime_list = [
            {
                ...
                'description': (anime.get('description') or '').replace('<br>', '').replace('<br/>', ''),
        ...
        # Handle error (could be network or API issue)
        anime_list = []

    return render(request, 'search.html', {'books': anime_list})
...
def view_recommendations(request):
    # Retrieve all recommendations ordered by publication date in descending order
    recommendations = Recommendation.objects.all().order_by('-publication_date')

    # If there are more than 10 recommendations, randomly select 10
    if recommendations.count() > 10:
        recommendations = random.sample(list(recommendations), 10)
...
def search_books_api(request):
    ...
    if query:
        params = {
            'q': query,
            'key': ANILIST_GRAPHQL_URL,
            'maxResults': 20 #LIMITED TO 10
        }
        
        try:
            response = requests.get(ANILIST_GRAPHQL_URL, params=params)
            response.raise_for_status()  
